...
class UserController extends Controller
{
    ...
    public function dashboard(){
      ...
      $upcominbirthday = $this->getAllBirthday();
        ...
        return view('dashboard', compact('data','attendance','upcominbirthday'));
    }
...
public function totalLeave()
{
    ...
    $joinDate = Carbon::parse($user->joining_date); // DOB नहीं, Joining Date लें
    $today = Carbon::today();
    ...
    // Employee अगर Financial Year के बाद join हुआ है
    $startDate = $joinDate->greaterThan($fyStart)
        ? $joinDate->copy()->startOfMonth()
        : $fyStart->copy();

    // Join अभी तक नहीं हुआ
    if ($joinDate->greaterThan($today)) {
        return 0;
    }

    // Months Count (Current Month Included)
