...
class UserController extends Controller
{
    ...
    public function sessionCheck($session_id,$user_id,$device_type){
       $prevSession =sessionCheck::where('user_id', $user_id)->where('last_activity',$session_id)->where('status' == '0' )->first();
       if($prevSession){
    return back()->with('error', 'already login in another device to login here please logout from there');
       }
