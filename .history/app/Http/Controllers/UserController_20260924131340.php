...
class UserController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($data && Hash::check($request->password, $data->password)) {
            ...
            $sessioncheck = $this->sessionCheck($session_id,$user_id,$device_type);
            if(!$sessioncheck){
                return redirect()-back()->with('error', 'already login in another device to login here please logout from there');
            }
    ...
    {
        $session_id = session()->getId();
        $session = sessionCheck::where('user_id',session('id') )->where('session_id', $session_id)->first();
        if($session){
    ...
    public function sessionCheck($session_id,$user_id,$device_type){
       $prevSession =sessionCheck::where('user_id', $user_id)->whereDate('last_activity',today())->where('status','0' )->first();
       if($prevSession){
       return false;
       }
       return sessionCheck::create([
        'user_id' => $user_id,
