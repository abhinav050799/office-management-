...
<script>
    ...
    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex){
        ...
        // Table Values
        var rowFromDate = data[3];   // From Date Column
        var rowStatus = $('<div>').html(data[7]).text().trim(); // Status Text

        // Date Compare
        if(fromDate){
            if(new Date(rowFromDate) < new Date(fromDate)){
                return false;
            }
        }

        if(toDate){
            if(new Date(rowFromDate) > new Date(toDate)){
                return false;
            }
        }
