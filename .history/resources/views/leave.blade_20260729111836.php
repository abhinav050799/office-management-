                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($data->status == 'Pending')
                                     ...
                                     @else
                                      <span class="badge bg-danger">{{ $data->status }}</span></td>
                                     @endif
                                <td>
                                    <button type="submit" class="btn btn-sm btn-primary me-1 editBtn" title="Edit" data-bs-toggle="modal"
            ...
            <form id="leaveForm" method="POST" action="">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label form-labelModal">Department</label>
                             <input type="text" class="form-control" name="department"  id="edit_department" value="" readonly>
                        ...
                        <div class="col-md-6">
                            <label class="form-label form-labelModal">Account Id</label>
                            <input type="text" class="form-control" name="account_id"  id="edit_account_id" value="" readonly>
                        </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label form-labelModal">Leave From</label>
                            <input type="date" class="form-control" name="leavefrom" id="leaveFrom edit_leavefrom" required>
                        </div>

                        <!-- Leave To -->
                        <div class="col-md-6">
                            <label class="form-label form-labelModal">Leave To</label>
                            <input type="date" class="form-control" name="leaveto" id="leaveTo edit_leaveto" required>
                        </div>

                        <!-- Leave Reason -->
                        <div class="col-md-12">
                            <label class="form-label form-labelModal" >Leave Reason</label>
                            <select class="form-control" name="leavereason" id="" required>
                                <option value="" selected>Reason</option>
                                <option value="urgentwork" >Urgent Work</option>
                                <option value="personalwork">Personal Work</option>
                                <option value="sickleave">Sick Leave</option>
...
<script>
  $(document).on('click', '.editBtn', function () {
    ...
    var id = $(this).data('id');   // <-- button se id le rahe hain

    $.ajax({
        url: "{{ route('getidData') }}",
        type: "GET",   // Route bhi GET hai
        data: {
            id: id
        },
        success: function(response){
            ...
            if(response.status){
                ...
                $('#edit_leavefrom').val(response.data.leavefrom);
                $('#edit_leaveto').val(response.data.leaveto);
                $('#edit_leavepurpose').val(response.data.leavepurpose);
                $('#edit_address').val(response.data.address);
                $('#edit_phone').val(response.data.phone);

                $('input[name="leaveType"][value="'+response.data.leavetype+'"]')
                    .prop('checked', true);

                $('select[name="leavereason"]').val(response.data.leavereason);
