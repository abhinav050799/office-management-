...
const rowFromDate = new Date(data[3]); // Table From Date
const rowToDate = new Date(data[4]);   // Table To Date
        const rowStatus = data[7].replace(/<[^>]*>/g, '').trim(); // HTML Remove

        if (fromDate) {
            if (rowDate < new Date(fromDate)) {
                return false;
            }
        }

        if (toDate) {
            if (rowDate > new Date(toDate)) {
                return false;
            }
        }
